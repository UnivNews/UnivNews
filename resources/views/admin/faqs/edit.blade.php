...
@section('title', 'Edit FAQ — University News')
@section('header_tagline', 'SITE CONTENT - EDIT FAQ')
...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold font-heading text-[#00081e] tracking-tight">Edit FAQ Item</h1>
            <p class="text-gray-500 font-sans text-sm mt-1">Update question, answer, category, or order.</p>
        </div>
        <div>
    ...
    <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" class="space-y-6">
        ...
        <div class="bg-white border border-gray-200 shadow-sm p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" id="question" name="question" value="{{ old('question', $faq->question) }}" class="w-full border border-gray-300 focus:border-crimson focus:ring-0 text-sm font-sans px-3 py-2" required>
            </div>
            ...
            <div>
                ...
                <textarea id="answer" name="answer" rows="6" class="w-full border border-gray-300 focus:border-crimson focus:ring-0 text-sm font-sans p-3" required>{{ old('answer', $faq->answer) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', $faq->order) }}" class="w-full border border-gray-300 focus:border-crimson focus:ring-0 text-sm font-sans px-3 py-2" min="0">
                </div>

                <div class="flex items-center pt-6">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" class="rounded border-gray-300 text-crimson focus:ring-crimson h-4 w-4" {{ old('is_published', $faq->is_published) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm font-medium text-navy">Published</span>
                    </label>
                </div>
            </div>
